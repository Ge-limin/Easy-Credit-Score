...
import numpy as np
from pseudo_encrypt import pseudo_encrypt
...
def preprocess_data(df):
    ...
    df['Num_Bank_Accounts'] = df['Num_Bank_Accounts']
    df['Num_Credit_Card'] = df['Num_Credit_Card']
    df['Interest_Rate'] = df['Interest_Rate']
...
def add_pseudo_encryption(df):
    df['Age'] = df['Age'].apply(lambda x: pseudo_encrypt(x) if pd.notnull(x) else x)
    df['Annual_Income'] = df['Annual_Income'].apply(lambda x: pseudo_encrypt(x) if pd.notnull(x) else x)
    df['Monthly_Inhand_Salary'] = df['Monthly_Inhand_Salary'].apply(lambda x: pseudo_encrypt(x) if pd.notnull(x) else x)
    df['Num_of_Loan'] = df['Num_of_Loan'].apply(lambda x: pseudo_encrypt(x) if pd.notnull(x) else x)
    df['Num_Bank_Accounts'] = df['Num_Bank_Accounts'].apply(lambda x: pseudo_encrypt(x) if pd.notnull(x) else x)
    df['Num_Credit_Card'] = df['Num_Credit_Card'].apply(lambda x: pseudo_encrypt(x) if pd.notnull(x) else x)
    df['Interest_Rate'] = df['Interest_Rate'].apply(lambda x: pseudo_encrypt(x) if pd.notnull(x) else x)
...
useful_features = [
    # These will be pseudo encrypted (ideally homomorphically but not in this code)
    # when user data passed into model
    'Age', 'Annual_Income', 'Monthly_Inhand_Salary',
    'Num_Bank_Accounts', 'Num_Credit_Card', 'Interest_Rate', 'Num_of_Loan',
    # categorical, not encrypted
    'Occupation',
    # These will be generated by MPC aggregated statistics
...
def train_model():
    ...
    # X.to_csv('train_encrypted_processed.csv', index=False)
    y = df['Credit_Score']
